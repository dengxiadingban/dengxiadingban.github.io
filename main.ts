...
// 生成易支付MD5签名
function generateSign(params: Record<string, any>, key: string): { sign: string; signString: string } {
...
Deno.serve(async (request: Request) => {
  ...
  try {
    ...
    const EPAY_API_URL = Deno.env.get('EPAY_API_URL');
    ...
    const params: Record<string, any> = {
      pid: EPAY_PID,
      type: data.type === 'wechat' ? 'wxpay' : data.type,
      out_trade_no: data.out_trade_no,
      notify_url: data.return_url,
    ...
    // 生成签名
    const signResult = generateSign(params, EPAY_KEY!);
